...
from logbert_pytorch.dataset import LogDataset
from logbert_pytorch.dataset.sample import fixed_window
...
class Predictor():
    ...
    @staticmethod
    def generate_dataset(line, window_size, adaptive_window, seq_len, scale, min_len):
        """
        :return: log_seqs: num_samples x session(seq)_length, tim_seqs: num_samples x session_length
        """
        log_seqs = []
        ...
        print(f"size: {len(log_seqs)}")
        return log_seqs, tim_seqs

    def predictInMemory(self, line):
        ...
        if self.hypersphere_loss:
            center_dict = torch.load(os.path.join(self.model_dir,"best_center.pt"), map_location=torch.device('cpu'))
            self.center = center_dict["center"]
            self.radius = center_dict["radius"]
            # self.center = self.center.view(1,-1)
        ...
        print("predicting...")
        #self.helper(model, self.output_dir, "test_normal", vocab, scale, error_dict)
        ...
        total_dist = []
        output_cls = []
        logkey_test, time_test = self.generate_dataset(line, self.window_size, self.adaptive_window, self.seq_len, scale, self.min_len)
        ...
        for idx, data in enumerate(data_loader):
            ...
            # Set an anomaly detection threshold (this would be derived from your data)
            anomaly_threshold = 0.8

            # Check if the log is anomalous
            #anomaly_scores = torch.softmax(logits, dim=-1)
            #is_anomalous = anomaly_scores.max(dim=-1).values > anomaly_threshold
            ...
            for i in range(len(data["bert_label"])):
                ...
                if self.hypersphere_loss_test:
                    ...
                    print(f"Radius:{self.radius}, Distance:{dist}")
        ...
        #for seq_th in seq_range:
        #    print(f"threshold:{seq_th}")
        #    total_errors = compute_anomaly(total_results, params, seq_th)
        #    print(f"total results:{total_errors}")
            
        total_errors = compute_anomaly(total_results, params, 0.3)
        print(f"total results:{total_errors}")
